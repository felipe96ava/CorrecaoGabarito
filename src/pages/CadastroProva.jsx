    ...
    <div className="max-w-3xl">
        ...
        <div className="mb-4 flex items-center justify-between bg-white rounded-xl shadow p-4">
          <div>
            ...
            <div className="text-sm text-gray-500">
              {(() => {
                if (!omrConfig?.box_bolhas_1_20) return 'Sem calibração. A leitura pode falhar dependendo da foto.';
                const total =
                  Number(prova?.total_questoes) ||
                  (Array.isArray(prova?.gabarito) ? prova.gabarito.length : 0) ||
                  0;
                const colsNec = Math.max(1, Math.min(4, Math.ceil(total / 20)));
                const colsCal = ['box_bolhas_1_20', 'box_bolhas_21_40', 'box_bolhas_41_60', 'box_bolhas_61_80']
                  .slice(0, colsNec)
                  .filter((k) => omrConfig?.[k])
                  .length;
                return `Calibração salva: ${colsCal}/${colsNec} coluna(s) com recorte explícito.`;
              })()}
            </div>
          </div>
          <button
      ...
      {id && (
        <ModalCalibrarOMR
          ...
          existingBoxes={{
            box_bolhas_1_20:  omrConfig?.box_bolhas_1_20  || null,
            box_bolhas_21_40: omrConfig?.box_bolhas_21_40 || null,
            box_bolhas_41_60: omrConfig?.box_bolhas_41_60 || null,
            box_bolhas_61_80: omrConfig?.box_bolhas_61_80 || null,
          }}
          colunasNecessarias={(() => {
            const total =
              Number(prova?.total_questoes) ||
              (Array.isArray(prova?.gabarito) ? prova.gabarito.length : 0) ||
              0;
            return Math.max(1, Math.min(4, Math.ceil(total / 20)));
          })()}
          saving={salvarOmr.isPending}
          onSave={async (boxes) => {
            await salvarOmr.mutateAsync(boxes);
            setOpenOmr(false);
