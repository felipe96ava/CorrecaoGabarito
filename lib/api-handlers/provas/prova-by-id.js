...
async function handler(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    const secoesSalvas = await sql`SELECT * FROM secoes WHERE prova_id = ${id} ORDER BY ordem`;
    return res.status(200).json({ data: { ...prova, secoes: secoesSalvas }, error: null });
  }
